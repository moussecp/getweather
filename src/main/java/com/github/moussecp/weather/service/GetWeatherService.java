...
import com.github.moussecp.weather.domain.Weather;
...
public class GetWeatherService {
    ...
    public Weather getWeather(Long locationId) throws JAXBException, IOException {
        Location location = locationRepository.find(locationId);
        if(location != null) {
            YahooWeatherService service = new YahooWeatherService();
            Channel channel = service.getForecast(location.getWoeid().toString(), DegreeUnit.CELSIUS);
            return new Weather(channel);
        } else {
            throw new IllegalArgumentException("Location with id: " + locationId + " cannot be found");
        }
    }
